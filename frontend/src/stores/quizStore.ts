...
interface QuizStore {
  ...
  updateQuizResults: (updates: Partial<PersonalityResponse>) => void;
...
export const useQuizStore = create<QuizStore>((set) => ({
  ...
  updateQuizResults: (updates) => set((state) => ({
    quizResults: state.quizResults ? { ...state.quizResults, ...updates } : null
  })),
