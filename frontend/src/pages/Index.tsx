  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-emerald-100/30 rounded-full blur-3xl transform -translate-x-1/2" />
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-yellow-100/30 rounded-full blur-3xl transform translate-x-1/2" />
      </div>
      ...
      <div className="flex flex-col items-center mb-20 relative">
        ...
        {/* Optionally add a subheading here if needed: */}
        {/* <div className="text-2xl md:text-3xl font-medium text-gray-500 text-center mb-8 leading-relaxed">Your subheading here</div> */}
      </div>
      ...
      <motion.div 
        ...
        viewport={{ once: true }}
        style={{ 
          background: 'linear-gradient(to bottom, transparent, rgba(243, 253, 248, 0.8))'
        }}
      />
    </section>
  );
...
const actionSpotlightData = [
  ...
  },
  {
    ...
    subtext: 'Supports farmers within 20km',
  ...
  },
  {
    ...
    cta: '📩 Ready to see your potential? Get a free solar estimate based on your address.',
  },
];
        ...
        {actionSpotlightData.map((card, idx) => (
          ...
          >
            ...
            <div className="w-full flex flex-col items-center mb-8">
              ...
              <h3 className="text-2xl md:text-3xl font-extrabold text-emerald-800 tracking-tight text-center">{card.title}</h3>
              {card.subtext && (
        ...
        >
          ...
          >
            ...
            <div className="relative w-full h-64 md:h-80 bg-gray-100">
              ...
              <button
                className="absolute top-4 right-4 text-gray-400 hover:text-emerald-600 text-3xl font-bold focus:outline-none"
            ...
            <div className="bg-white/95 p-8 flex flex-col gap-6">
              <div className="text-emerald-600 text-lg font-semibold uppercase tracking-wide">Real stories. Small shifts. Lasting change.</div>
              <div className="text-2xl md:text-3xl font-extrabold text-emerald-800">{modalCard.title}</div>
              {modalCard.impactStat && (
                <div className="text-lg md:text-xl font-semibold text-emerald-700">{modalCard.impactStat}</div>
              )}
              {modalCard.whyItMatters && (
                <div className="italic text-gray-700">{modalCard.whyItMatters}</div>
              )}
              {modalCard.cta && (
                <div className="bg-emerald-100 text-emerald-800 font-bold rounded-full px-6 py-3 text-center">{modalCard.cta}</div>
              )}
            </div>
          </motion.div>
        </motion.div>
