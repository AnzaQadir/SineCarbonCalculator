...
exports.getNextActionsEnhanced = getNextActionsEnhanced;
 ...
 */
function cardToCatalogItem(card, record) {
    ...
    if (record) {
        ...
        const recordEffort = record.effort || {};
        const metadata = record.metadata || {};
        const fallbackSteps = card.behaviorDistance === 'small' ? 2 : card.behaviorDistance === 'medium' ? 4 : 6;
        const fallbackMinutes = card.behaviorDistance === 'small' ? 3 : card.behaviorDistance === 'medium' ? 10 : 30;
        return {
            id: record.id,
            ...
            subtitle: record.subtitle ||
                ...
                card.action,
            metrics: {
                pkrMonth: typeof recordMetrics.pkrMonth === 'number'
                    ? recordMetrics.pkrMonth
                    : estimatedRupees,
                minutes: typeof recordMetrics.minutes === 'number'
                    ? recordMetrics.minutes
                    : fallbackMinutes,
                kgco2eMonth: typeof recordMetrics.kgco2eMonth === 'number'
                    ? recordMetrics.kgco2eMonth
                    : weeklyCo2 * 4.33,
            },
            effort: {
                steps: typeof recordEffort.steps === 'number' ? recordEffort.steps : fallbackSteps,
                requiresPurchase: typeof recordEffort.requiresPurchase === 'boolean'
                    ? recordEffort.requiresPurchase
                    : card.prerequisites.some((p) => p.toLowerCase().includes('buy') || p.toLowerCase().includes('purchase')),
                avgMinutesToDo: typeof recordEffort.avgMinutesToDo === 'number'
...
function buildRecommendationDetails(scored, record, card) {
    ...
    const effort = record?.effort || scored.item.effort || {};
 ...
 */
async function getRecentEvents(userId, eventType, limit = 60) {
 ...
 */
async function getDoneTodayIds(userId, today) {
 ...
 */
function scoredItemToNextAction(scored, type, rules, persona, personality, context) {
    const record = context?.catalogRecordMap?.get(scored.item.id);
    const card = context?.cardMap?.get(scored.item.id) || (0, recommendationCatalogService_1.queryCatalog)({}).find((c) => c.id === scored.item.id);
    ...
    const subtitle = record?.subtitle ||
        ...
        scored.item.subtitle ||
        card?.levels?.start;
    const utilitySource = recordMetadata?.utilityModel?.source || 'ZERRAH Catalog v1.0';
    ...
    const learnSummary = recommendation.why ||
        recordMetadata?.why ||
        card?.why ||
 ...
 */
async function getNextActionsEnhanced(userId) {
    ...
    const persExtended = await models_1.UserPersonalityExtended.findByPk(userId);
    // Get archetype scores (from existing personality or defaults)
    const archetypeScores = persExtended?.archetypeScores || {};
    ...
    // 2) Persona vector
    const persona = persExtended?.personaVector || (0, persona_1.archetypesToPersona)(archetypeScores);
    // 3) Context knobs
    const ctx = {
        sprintWeek: (0, timezone_1.isSprintWeek)(userId),
        monthEnd: (0, timezone_1.isMonthEnd)(),
    };
    const W = persExtended?.weightPrefs
        ? persExtended.weightPrefs
        : (0, weights_1.weightsFromPersona)(persona, ctx);
    // 4) Load rules
    ...
    const today = (0, timezone_1.startOfTodayInKarachi)();
    ...
    let catalogRecordMap = new Map();
    if (candidates.length > 0) {
        const candidateIds = candidates.map((card) => card.id);
        const records = await models_1.RecommendationCatalog.findAll({
            where: {
                id: {
                    [sequelize_1.Op.in]: candidateIds,
                },
                active: true,
            },
        });
    ...
    const catalogItems = candidates.map((card) => cardToCatalogItem(card, catalogRecordMap.get(card.id)));
    console.log(`[Enhanced Engagement] Catalog items after all filtering: ${catalogItems.length}`);
    if (catalogItems.length === 0) {
        ...
        console.error(`[Enhanced Engagement] Debug info:`, {
            ...
            totalCatalogSize: (0, recommendationCatalogService_1.queryCatalog)({}).length,
        });
    ...
    const scored = catalogItems
        .map((item) => {
        const res = (0, ranker_1.scoreAction)({
            locale: user.city || 'PK',
            ...
            dismissed,
            snoozed,
        }, item, W, shownByCategory);
        return res;
    })
        .sort((a, b) => b.score - a.score);
    console.log(`[Enhanced Engagement] Scored ${scored.length} items. Top 3 scores:`, scored.slice(0, 3).map(s => ({ id: s.item.id, score: s.score.toFixed(3) })));
    // 8) Pick 1 primary + 2 alternates (MMR for diversity)
    const top3 = (0, ranker_1.rankMMR)(scored, 3, 0.75);
    ...
    if (top3.length === 0) {
        ...
        if (scored.length > 0) {
            ...
            const primary = topScored[0]
                ...
                : null;
            const alternatives = topScored.slice(1).map((s, i) => scoredItemToNextAction(s, i === 0 ? 'quick_win' : 'level_up', rules, persona, personality, {
                catalogRecordMap,
                cardMap: candidateCardMap,
            }));
            // Log SHOWN events
            await Promise.all(topScored.map((x) => models_1.UserActionEvent.create({
                userId,
    ...
    const alt2 = a2Scored
        ...
        : null;
    // 10) Log SHOWN events
    await Promise.all(top3.map((x) => models_1.UserActionEvent.create({
        userId,
        recommendationId: x.item.id,
        eventType: 'SHOWN',
        occurredAt: new Date(),
    ...
    if (!primaryScored) {
        ...
        if (a1Scored) {
            const primary = scoredItemToNextAction(a1Scored, 'best', rules, persona, personality, {
                catalogRecordMap,
                cardMap: candidateCardMap,
            });
            const alternatives = [];
            if (a2Scored) {
                ...
                alternatives.push(scoredItemToNextAction(a2Scored, type, rules, persona, personality, {
                    catalogRecordMap,
                    cardMap: candidateCardMap,
                }));
            }
    ...
    const primary = scoredItemToNextAction(primaryScored, 'best', rules, persona, personality, {
        catalogRecordMap,
        cardMap: candidateCardMap,
    });
    const alternatives = [];
    if (alt1) {
        alternatives.push(scoredItemToNextAction(alt1, alt1.type, rules, persona, personality, {
            catalogRecordMap,
            cardMap: candidateCardMap,
        }));
    }
    if (alt2) {
        alternatives.push(scoredItemToNextAction(alt2, alt2.type, rules, persona, personality, {
            catalogRecordMap,
            cardMap: candidateCardMap,
        }));
    }
