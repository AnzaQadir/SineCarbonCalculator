...
exports.PersonalityRecommendationService = void 0;
...
class PersonalityRecommendationService {
    /**
     * Get recommendations for a specific personality type
     */
    static getRecommendationsForPersonality(personalityType) {
        return personalityRecommendations.filter(rec => rec.personas.includes(personalityType));
    }
    /**
     * Get recommendations by category
     */
    static getRecommendationsByCategory(category) {
        return personalityRecommendations.filter(rec => rec.category === category);
    }
    /**
     * Get recommendations by effort level
     */
    static getRecommendationsByEffortLevel(effortLevel) {
        return personalityRecommendations.filter(rec => rec.effortLevel === effortLevel);
    }
    /**
     * Get recommendations by lifestyle
     */
    static getRecommendationsByLifestyle(lifestyle) {
        return personalityRecommendations.filter(rec => rec.lifestyle === lifestyle);
    }
    /**
     * Get personalized recommendations based on personality and preferences
     */
    static getPersonalizedRecommendations(personalityType, preferences = {}) {
        let recommendations = this.getRecommendationsForPersonality(personalityType);
        if (preferences.effortLevel) {
            recommendations = recommendations.filter(rec => rec.effortLevel === preferences.effortLevel);
        }
        if (preferences.category) {
            recommendations = recommendations.filter(rec => rec.category === preferences.category);
        }
        if (preferences.lifestyle) {
            recommendations = recommendations.filter(rec => rec.lifestyle === preferences.lifestyle);
        }
        return recommendations;
    }
}
exports.PersonalityRecommendationService = PersonalityRecommendationService;
