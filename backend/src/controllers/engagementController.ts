...
export class EngagementController {
  ...
  private static async getUserId(req: AuthenticatedRequest): Promise<string | null> {
    if (req.userId) {
      return req.userId;
    }
    if (req.userEmail) {
  ...
  static async getNextActions(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const actions = await EngagementService.getNextActions(userId);
  ...
  static async getBestNextAction(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = await EngagementController.getUserId(req);
      ...
      const action = await EngagementService.getBestNextAction(userId);
  ...
  static async actionDone(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = await EngagementController.getUserId(req);
      ...
      const result = await EngagementService.recordActionDone(userId, recommendationId, context);
      res.json(result);
    } catch (error) {
  ...
  static async getHomeFeed(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = await EngagementController.getUserId(req);
      ...
      const bestNextAction = await EngagementService.getBestNextAction(userId);
      ...
      const sections = [
        {
          title: 'Quick Wins',
          cards: allCards.slice(0, 3).map((card: any) => ({
            id: card.id,
            title: card.action,
            domain: card.domain,
            actFirst: true,
        ...
        {
          title: 'Money Savers',
          cards: allCards.slice(3, 6).map((card: any) => ({
            id: card.id,
            title: card.action,
            domain: card.domain,
            actFirst: true,
  ...
  static async getWeeklyRecap(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = await EngagementController.getUserId(req);
      ...
      const recap = await EngagementService.getWeeklyRecap(userId);
