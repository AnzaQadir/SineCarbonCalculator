...
import { EngagementRules } from '../types/engagement';

const DEFAULT_RULES: EngagementRules = {
...
let cachedRules: EngagementRules | null = null;
let lastFetchTime: number = 0;
const CACHE_TTL_MS = 60000; // 60 seconds

export class EngagementRuleOverlayService {
  /**
   * Load rules from database or use defaults
   */
  static async loadRules(): Promise<EngagementRules> {
    const now = Date.now();
    ...
    try {
      const config = await AppConfig.findByPk('engagement_rules_v1');
      if (config && config.value) {
        cachedRules = config.value as EngagementRules;
        lastFetchTime = now;
        return cachedRules;
      }
    } catch (error) {
   ...
   */
  static async updateRules(rules: EngagementRules): Promise<void> {
    try {
   ...
   */
  static getPriorityOrder(rules: EngagementRules, personality?: string): string[] {
    if (personality) {
  ...
  static async initializeDefaults(): Promise<void> {
    try {
      const existing = await AppConfig.findByPk('engagement_rules_v1');
      if (!existing) {
